			...
			<div class="row">
				<div class="col-lg-12">
					<div class="heading-title text-center">
						<h2>@lang('Registrate')</h2>
						<p>@lang('Create una cuenta y ten acceso a nuestros servicios')</p>
					</div>
				</div>
			</div>
			<div class="row">
						...
						<div class="row">
							<div class="col-md-12">
								<div class="form-group">
									<select id="rules_id"  name ="rules_id" class="custom-select d-block form-control" required data-error="Selecciones el Rol">
										...
										♠        @if($rule['id']==1)
										             <option selected value="{{$rule['id']}}">{{$rule['rule']}}</option>
										          @endif
							...
							<script >
								$("#departament").change(event => {
	                                 $.get(`/register/create/${event.target.value}`, function(res, sta){
		                                   $("#province").empty();
		                                   $("#city").empty();
							...
							<script>
								$("#province").change(event => {
	                               $.get(`/register/create/city/${event.target.value}`, function(res, sta){
		                           $("#city").empty();
