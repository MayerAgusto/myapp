				...
				<div class="col-lg-12">
					<form action="{{url('/users/'.$data->id)}}" method="post" enctype="multipart/form-data" >
						...
						<div class="row">
								...
								<div class="form-group">
									<select id="rules_id"  name ="rules_id" class="custom-select d-block form-control" required data-error="Selecciones el Rol">
										...
										@foreach($rules as $rule)
 											 @if($rule->id == $data->rules_id)
	   											<option value="{{$rule['id']}}" selected>{{$rule['rule']}}</option>
											@endif
											@if($rule->id != $data->rules_id)
							...
							<div class="col-md-12">
								<div class="form-group">
									<input type="text" class="form-control" id="name" name="name" value="{{$data->name}}" placeholder="Nombre" required data-error="Ingrese su nombre">
									<div class="help-block with-errors"></div>
								</div>                                 
							</div>
							<div class="col-md-12">
								<div class="form-group">
									...
									value="{{$data->email}}" placeholder="Correo Electronico" required data-error="Ingrese su correo Electronico">
									<div class="help-block with-errors"></div>
								</div> 
							</div>
							...
							<div class="col-md-12">
								<div class="form-group">
									...
									placeholder="Clave" required data-error="Ingrese su clave">
									<div class="help-block with-errors"></div>
								</div> 
							</div>
								...
								<div class="form-group">
									<select id="province"  name ="province" class="custom-select d-block form-control" required data-error="Selecciones su provincia">
										...
										@foreach($provincias as $province)
											@if($province->province == $data->province)
								...
								<div class="form-group">
									<select id="city"  name ="city" class="custom-select d-block form-control"  required data-error="Seleccione su ciudad">
									  ...
									  @foreach($distritos as $distrito)
											@if($distrito->city == $data->city)
	                                             <option value="{{$distrito['city']}}" selected>{{$distrito['city']}}</option>
	                                        @endif
	                                        @if($distrito->city != $data->city)
							...
							<div class="col-md-12">
								<div class="form-group">
									...
									<input type="file"  id="image" name="image" class="form-control" placeholder="Imagen" value="{{$data->image}}">
									<div class="help-block with-errors"></div>
								</div>                                 
							</div>
							<div class="col-md-12">
